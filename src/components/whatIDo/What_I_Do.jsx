...
const WhatIDo = () => {
  ...
  const cardStyles = [
    { bg: 'bg-[#F0F7FF]', border: 'border-blue-100', accent: 'text-blue-600' },
    { bg: 'bg-[#F5F3FF]', border: 'border-purple-100', accent: 'text-purple-600' },
    { bg: 'bg-[#F0FDF4]', border: 'border-emerald-100', accent: 'text-emerald-600' },
    { bg: 'bg-[#FFF7ED]', border: 'border-orange-100', accent: 'text-orange-600' },
    { bg: 'bg-[#FFF1F2]', border: 'border-rose-100', accent: 'text-rose-600' },
  ];

  return (
    <section className="w-full bg-[#f5d27f] font-poppins py-8 relative overflow-hidden">
      
      <div className="relative z-10 max-w-[1440px] mx-auto px-4 md:px-6">
        
        {/* Compact Header */}
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center gap-2 mb-2 px-3 py-1 rounded-full bg-[#eeab0e] shadow-sm border border-gray-100 text-[9px] font-bold tracking-[0.2em] text-white uppercase">
            <Sparkles size={10} className="text-white" /> Interior Solutions
          </div>
          <h2 className="text-2xl md:text-5xl font-semibold text-gray-900 tracking-tight">
            Our <span className="text-[#b07800e5] font-light italic">Design</span> Services
          </h2>
          <div className="w-12 h-1 bg-red-900 mx-auto mt-3 rounded-full opacity-70"></div>
        </div>

        {/* Responsive Grid: Mobile(2), Tablet(4), Desktop(5) */}
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-6">
          {solutions.map((item, index) => {
            ...
            return (
              <div 
                ...
                className={`group relative p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border transition-all duration-300 cursor-pointer shadow-sm hover:shadow-xl hover:-translate-y-1 ${style.bg} ${style.border}`}
              >
                {/* Icon Section (Plus Hata Diya Gaya Hai) */}
                <div className="flex justify-center md:justify-start items-center mb-6 md:mb-10">
                ...
                <div className="text-center md:text-left">
                  <h3 className="text-sm md:text-lg font-bold text-gray-800 leading-tight">
                    {item.title}
                  </h3>
                  {/* View Gallery Redirect Logic */}
                  <div 
                    onClick={(e) => {
                        e.stopPropagation(); // Card modal open hone se rokne ke liye
                        navigate('/gallery');
                    }}
                    className="mt-2 flex items-center justify-center md:justify-start gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300"
                  >
                    <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-wider ${style.accent}`}>View Gallery</span>
      ...
      {activeCategory && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="bg-white w-full max-w-5xl max-h-[85vh] rounded-[2rem] shadow-2xl relative z-10 overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b flex justify-between items-center bg-white sticky top-0 z-20">
              <div>
                <p className="text-[9px] font-bold text-red-500 uppercase tracking-[0.2em]">Portfolio</p>
                <h3 className="text-lg md:text-2xl font-bold text-gray-900">{selectedData.title}</h3>
              </div>
              <button 
                onClick={() => setActiveCategory(null)} 
                className="p-2 bg-gray-100 hover:bg-red-500 hover:text-white rounded-full transition-all duration-300"
              >
                <X size={18} />
              </button>
            </div>
